...
| Read-only review target POC | Actually implemented as hidden demo screen | `frontend/src/pages/ReviewPage.tsx`, `frontend/src/App.tsx` | `?review=1` shows HITL/UNVERIFIED/RUMOR/INSIGHT candidates with source links. It does not update Supabase and is not an admin approval system. |
