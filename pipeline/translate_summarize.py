...
# Post-process helper
# ────────────────────────────────────────────────
def _apply_post_process(en_text: str, result: dict) -> dict:
    """번역 결과에 TPR 후처리(고유명사 복원 + 누락 용어 주입)를 적용한다."""
...
def translate_and_summarize(
    ...
    if LLM_PROVIDER in ("openrouter", "cloud"):
        try:
            result = _translate_openrouter(system, user_content, temperature)
            return _apply_post_process(text, result)
        except Exception as exc:
    ...
    if LLM_PROVIDER == "gemini":
        try:
            result = _translate_gemini(system, user_content, temperature)
            return _apply_post_process(text, result)
        except Exception as exc:
            if DISABLE_LOCAL_FALLBACK:
                raise
