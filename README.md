...
발표용으로는 숨겨진 읽기 전용 POC 화면 `?review=1`에서 `HITL_REQUIRED`, `UNVERIFIED`, `RUMOR`, `INSIGHT` 검토 대상 목록을 확인할 수 있습니다. 이 화면은 검토 대상 분리를 보여주기 위한 POC이며, Supabase 데이터를 수정하거나 관리자 승인/반려를 수행하지 않습니다.
